...
class KNearestNeighbor(object):
    ...
    # Method to predict labels of test examples using chosen value of k given Manhattan distances obtained from 'compute_distances' method.
    def predict_labels(self, dists, k=1):
         num_test = dists.shape[0]
         y_pred = np.zeros(num_test)
         for i in range(num_test):
            closest_y = self.y_train[np.argsort(dists[i])[:k]]  # Only sort k smallest distances
            y_pred[i] = np.argmax(np.bincount(closest_y))  # Majority voting
